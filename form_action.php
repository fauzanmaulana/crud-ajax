...
if ($id != "") {
    $querysql = "UPDATE siswa SET nama = ?, alamat = ?, jurusan = ?, jenis_kelamin = ?, tanggal = ? WHERE siswa.id = ?;";
    $prepare = $conn->prepare($querysql);
    $prepare->bind_param("sssssi", $nama_siswa, $alamat, $jurusan, $kel, $tanggal_masuk, $id);
    $prepare->execute();
    echo $querysql;
} else {
    $querysql = "INSERT INTO `siswa` (`nama`, `alamat`, `jurusan`, `jenis_kelamin`, `tanggal_masuk`) VALUES (?, ?, ?, ?, ?);";
    $prepare = $conn->prepare($querysql);
